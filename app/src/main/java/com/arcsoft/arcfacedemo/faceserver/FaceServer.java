...
public class FaceServer {
    ...
    //特征值对应的文件名
    public  static String featureFileName=null;
    ...
    private void initFaceList(Context context) {
        synchronized (this) {
            if (ROOT_PATH == null) {
                ROOT_PATH = Environment.getExternalStorageDirectory() + File.separator + "arcface";
            }
    ...
    public CompareResult getTopOfFaceLib(FaceFeature faceFeature) {
        ...
        for (int i = 0; i < faceRegisterInfoList.size(); i++) {
            ...
            System.out.println("faceRegisterInfoList.get(i).getName()：：：：：："+faceRegisterInfoList.get(i).getName());
            if (faceSimilar.getScore() > maxSimilar) {
                maxSimilar = faceSimilar.getScore();
                //获取到特征值存储的文件名字
                featureFileName=FaceServer.ROOT_PATH+ File.separator +FaceServer.SAVE_FEATURE_DIR+File.separator+faceRegisterInfoList.get(i).getName() ;
                System.out.println("faceRegisterInfoList.get(i).getName()：：：：：："+FaceServer.ROOT_PATH+ File.separator +FaceServer.SAVE_FEATURE_DIR+File.separator+faceRegisterInfoList.get(i).getName());
                maxSimilarIndex = i;
            }
